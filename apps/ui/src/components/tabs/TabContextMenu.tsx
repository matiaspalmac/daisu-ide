    ...
    <ContextMenu.Root>
      ...
      <ContextMenu.Portal>
        <ContextMenu.Content className="daisu-cmenu" collisionPadding={8}>
          {item("Close", "close")}
          {item("Close Others", "closeOthers", props.totalTabs <= 1)}
          {item("Close All", "closeAll", props.totalTabs === 0)}
          <ContextMenu.Separator className="daisu-cmenu-separator" />
          {props.pinned ? item("Unpin", "unpin") : item("Pin", "pin")}
          <ContextMenu.Separator className="daisu-cmenu-separator" />
          {item("Copy Path", "copyPath", !props.hasPath)}
          {item("Reveal in File Explorer", "revealInExplorer", !props.hasPath)}
        </ContextMenu.Content>
      </ContextMenu.Portal>
    </ContextMenu.Root>
  );
